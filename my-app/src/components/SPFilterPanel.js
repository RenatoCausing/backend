...
const SPFilterPanel = ({ onSPSelect, showUploadButton, onUploadClick }) => {
  ...
  const [selectedDepartment, setSelectedDepartment] = useState(''); // This state will now hold the facultyId
  ...
  const SPApiService = {
    ...
    applyFilters: async (filters) => {
      const { adviserIds, tagIds, facultyId, searchTerm } = filters;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        let initialFacultyId = '';
        if (currentUser && currentUser.role === 'faculty' && currentUser.facultyId) {
          initialFacultyId = currentUser.facultyId;
          console.log(`Initial faculty filter determined: ${initialFacultyId}`);
          setSelectedDepartment(initialFacultyId); // Set the dropdown value
        } else {
            setSelectedDepartment(''); // Ensure it's reset if not faculty
        }

        // Apply filters immediately based on user role
        const initialFilters = {
          adviserIds: [],
          tagIds: [],
          facultyId: initialFacultyId, // Use the determined faculty ID here
          searchTerm: ''
        };
        const spData = await SPApiService.applyFilters(initialFilters);
        console.log('SPs fetched and initially filtered successfully:', spData);

        const sortedSpData = sortSPs(spData || []);
        setSps(spData || []); // Keep all fetched SPs if needed for other operations, but filteredSps is what's displayed
    ...
    fetchData();
  }, [refreshTrigger, currentUser]); // Keep currentUser in dependencies
  ...
  >
    <i className="fa fa-upload"></i> UPLOAD
              ...
              <select
                ...
                disabled={isAnyLoading || (currentUser?.role === 'faculty' && currentUser?.facultyId)}
              >
